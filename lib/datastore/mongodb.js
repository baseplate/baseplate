...
class MongoStore {
  ...
  static transformObjectIds(input, op = 'encode') {
    if (!isPlainObject(input)) {
      if (op === 'encode' && input instanceof EntryId) {
  ...
  async find({fieldSet, filter, modelName, pageNumber = 1, pageSize}) {
    ...
    const encodedQuery = MongoStore.transformObjectIds(query, 'encode')
  ...
  async findManyById({fieldSet, filter, ids, modelName}) {
    const connection = await this.connect()
    const collectionName = MongoStore.getCollectionName({modelName})
    const encodedIds = MongoStore.transformObjectIds(ids, 'encode').filter(
      Boolean
    )
    const options = {
      projection: MongoStore.getProjectionFromFieldSet(fieldSet)
