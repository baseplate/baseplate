class JsonApiResponse {
    ...
    if (this.entries) {
      ...
      try {
        const formattedEntries = await Promise.all(
          entriesArray.map(entry => this.formatEntry(entry))
        )
      ...
      response.included = await Promise.all(
        this.includedReferences.map(referenceValue => {
          if (Array.isArray(referenceValue)) {
            return Promise.all(referenceValue.map(this.formatEntry.bind(this)))
          }

          return this.formatEntry(referenceValue)
        })
      )
    }
  ...
  async formatEntry(entry) {
    const fields = await entry.toObject({fieldSet: this.fieldSet})
    const attributes = {}
    const meta = {}
    const relationships = {}
