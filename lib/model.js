...
const INTERNAL_FIELDS = ['_createdAt', '_updatedAt']
...
class Model {
  ...
  static async getAccessForUser({accessType, includePublicUser = true, user}) {
    ...
    const entries = await this.datastore.getAccess({
      includePublicUser,
      user
    })
    const modelEntries = entries.filter(entry => entry.model === this.name)
    const accessTypeEntries = modelEntries.map(modelEntry => {
      const value = (modelEntry && modelEntry[accessType]) || false

      return AccessValue.parse(value, {queryPrefix: '_'})
    })

    return AccessValue.unite.apply(null, accessTypeEntries)
  }
  ...
  hydrate(fields, {fromDb}) {
    ...
    Object.entries(fields).forEach(([name, value]) => {
      if (name === '_id') {
        ...
        if (value && schema.isReferenceField(name)) {
          this._fields[name] = Array.isArray(value)
            ? value.map(item => ({...item, _id: new EntryId(item._id)}))
            : {...value, _id: new EntryId(value._id)}
        } else {
