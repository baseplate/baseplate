...
const extendModelWithGraphQL = require('../extendModel')
    ...
    const resolve = async (root, args, context) => {
      ...
      const referenceModels = referencesArray.map(
        async ({_id: id, _type: type}) => {
          ...
          const ReferencedModel = modelFactory(referenceSchema, {context})
          const ReferencedGraphQLModel = extendModelWithGraphQL(ReferencedModel)
          const access = await ReferencedGraphQLModel.getAccessForUser({
            accessType: 'read',
            user: context.user
          })
